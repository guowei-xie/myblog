...
最优海报是第 `r best_arm` 张，真实辐射率 $p^{\star} = `r sprintf('%.2f', p_star)`$。把 5 张海报的真值画出来：

```{r fig-arms-truth, fig.height=3.4}
...
```{r fig-mc-egreedy, fig.height=3.9}
...
- **最优动作比例**稳步爬升——但受限于那固定的 $\epsilon$，它的天花板大约在 $1-\epsilon+\epsilon/K \approx `r sprintf('%.2f', 1-0.1+0.1/N_ARMS)`$，因为哪怕早已看清优劣，仍有 10% 的流量在随机乱试；
- **累计辐射率**逼近 $p^{\star}=`r sprintf('%.2f', p_star)`$，但始终差一口气——同样是那 10% 探索在往一号位塞次优海报；
...
```{r fig-mc-closearms, fig.height=3.9}
...
```{r fig-mc-ucb-vs-eg, fig.height=3.9}
...
```{r fig-cfactor-bound, fig.height=3.6}
...
```{r fig-mc-cfactor, fig.height=3.9}
...
```{r fig-mc-warmstart, fig.height=3.9}
set.seed(11)
