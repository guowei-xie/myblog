...
三个点的位置各不相同：A 在最右端（$x$ 极端），但正好落在其余点连成的那条线上；B 的 $x$ 很普通，$y$ 却高高抬起；C 的 $x$ 偏大，$y$ 又明显偏低。看实线（全量）和虚线（剔除 C）之间的夹角：只去掉 C 一个点，斜率就从 1.05 变到 1.33。一个点能不能左右回归线，和它残差大不大是两回事。
...
B 冲出了 $\pm 2.5$ 的阈值线（3.26），C 更低，到 -4.88。但 A 的标准化残差只有 0.18，完全正常。标准化残差只看 $y$ 方向的偏离，藏在 $x$ 极端处的高杠杆点 A 它是看不出来的。
...
这次 C 遥遥领先（$D_C =$ 1.74），把阈值线远远甩在下面；B 只是勉强越线，残差大但杠杆低；杠杆最高的 A 库克距离几乎为 0（0.008），因为它顺着趋势，删不删都不改变方程。可见高杠杆并不等于强影响。
...
斜率从 1.05 抬到 1.33（约 +26%），截距也明显回落。一个点就能改变"$x$ 每增加一个单位、$y$ 平均变化多少"这个结论。样本量小的时候，这种点必须查清楚：是录入错误，是量纲不一致，还是本就不该纳入的异常记录。
...
这张图有意思的地方在 A：前面几个指标都放过了它，只有 COVRATIO 把它标了出来。A 的 COVRATIO 有 1.56（在带子上方），说明这个高杠杆点其实在帮着稳住估计，删了反而更不准，是个"好"的杠杆点。C 则掉到 0.21（在带子下方很远），删掉它精度会明显变好，是要处理的坏点。
