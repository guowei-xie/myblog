...
三个点位置各异：A 在最右端（$x$ 极端）但落在其余点连成的线上；B 的 $x$ 普通、$y$ 却高高抬起；C 的 $x$ 偏大、$y$ 又明显偏低。看实线（全量）与虚线（剔除 C）的夹角：只去掉 C，斜率就从 `r sprintf("%.2f", coef(fit)[2])` 变到 `r sprintf("%.2f", coef(fit_noC)[2])`。能否左右回归线，和残差大小是两回事。
...
B 冲出 $\pm 2.5$ 阈值线（`r sprintf("%.2f", d$std_resid[d$label=="B"])`），C 更低至 `r sprintf("%.2f", d$std_resid[d$label=="C"])`；A 只有 `r sprintf("%.2f", d$std_resid[d$label=="A"])`，完全正常。标准化残差只看 $y$ 方向偏离，抓不到藏在 $x$ 极端处的高杠杆点 A。
...
C 遥遥领先（$D_C =$ `r sprintf("%.2f", d$cooks[d$label=="C"])`），把阈值线远远甩下；B 残差大但杠杆低，仅勉强越线；杠杆最高的 A 因顺着趋势，库克距离几乎为 0（`r sprintf("%.3f", d$cooks[d$label=="A"])`）。高杠杆不等于强影响。
...
斜率从 `r sprintf("%.2f", coef(fit)[2])` 抬到 `r sprintf("%.2f", coef(fit_noC)[2])`（约 +`r sprintf("%.0f", (coef(fit_noC)[2]-coef(fit)[2])/abs(coef(fit)[2])*100)`%），截距也明显回落。一个点就能改写"$x$ 每增一单位、$y$ 平均变化多少"的结论。样本量小时这种点必须查清：录入错误、量纲不一致，还是本不该纳入的异常记录。
...
这张图的看点在 A：前面几个指标都放过了它，只有 COVRATIO 标出它。A 的 COVRATIO 为 `r sprintf("%.2f", d$covratio[d$label=="A"])`（带子上方），说明这个高杠杆点在稳住估计、删了反而更不准，是"好"杠杆点。C 则掉到 `r sprintf("%.2f", d$covratio[d$label=="C"])`（带子下方很远），删掉它精度明显变好，是要处理的坏点。
