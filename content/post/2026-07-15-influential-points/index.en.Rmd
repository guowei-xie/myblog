...
三个点各司其职：**A** 在最右端（$x$ 极端），但正好落在其余点隐含的那条线上；**B** 的 $x$ 很普通，却高高地偏离；**C** 的 $x$ 偏大、$y$ 又明显偏低。注意实线（全量）与虚线（剔除 C）之间的夹角——只删掉 C 一个点，斜率就从 `r sprintf("%.2f", coef(fit)[2])` 跳到 `r sprintf("%.2f", coef(fit_noC)[2])`。这正是书中图 4-5 想说的：**一个点能不能左右回归线，和它残差大不大，是两回事。**
...
**B** 冲出了 $\pm 2.5$ 的红线（`r sprintf("%.2f", d$std_resid[d$label=="B"])`），**C** 更是低到 `r sprintf("%.2f", d$std_resid[d$label=="C"])`。但请注意 **A**：它的标准化残差只有 `r sprintf("%.2f", d$std_resid[d$label=="A"])`，完全正常。**标准化残差只看 $y$ 方向的偏离，对"藏在 $x$ 极端处"的高杠杆点 A 视而不见。**
...
结果一目了然：**C 一骑绝尘**（$D_C =$ `r sprintf("%.2f", d$cooks[d$label=="C"])`），把阈值线远远甩在下面；**B** 只是勉强越线（残差大但杠杆低）；而杠杆最高的 **A**，库克距离几乎是 0（`r sprintf("%.3f", d$cooks[d$label=="A"])`）——**因为它顺着趋势，删掉它方程根本不动。** 这就是"高杠杆 ≠ 强影响"最干净的证据。
...
斜率从 `r sprintf("%.2f", coef(fit)[2])` 抬到 `r sprintf("%.2f", coef(fit_noC)[2])`（约 +`r sprintf("%.0f", (coef(fit_noC)[2]-coef(fit)[2])/abs(coef(fit)[2])*100)`%），截距也大幅回落——**单单一个点，就足以改写"$x$ 每增加一个单位、$y$ 平均变化多少"这个核心结论。** 在小样本里，这种事必须查清楚：是录入错误？是量纲不一致？还是一笔本就不该纳入的异常记录？
...
这张图最有意思：前面所有指标都对 **A** 网开一面，唯独 COVRATIO 把它拎了出来——A 的 COVRATIO 高达 `r sprintf("%.2f", d$covratio[d$label=="A"])`（远在带子上方），意思是**这个高杠杆点其实在帮你稳住估计，删了反而更不准**，是名副其实的"好杠杆点"。而 **C** 掉到 `r sprintf("%.2f", d$covratio[d$label=="C"])`（带子下方很远），删掉它精度会明显改善——**坏点实锤**。
